...
namespace IoT.Simulator.Services
{
    public class DeviceSimulationService : ISimulationService
    {
        ...
        public DeviceSimulationService(IOptions<DeviceSettings> deviceSettings, ITelemetryMessageService telemetryMessagingService, ILoggerFactory loggerFactory)
        {
            ...
            _telemetryInterval = _simulationSettings.TelemetryTimeInterval;
        ...
        internal async Task SendDeviceToCloudMessagesAsync(string deviceId)
        {
            ...
            using (_logger.BeginScope($"{logPrefix}::{DateTime.Now}::{_deviceSettings.ArtifactId}::DATA"))
            {
                ...
                {
                    //Randomize data
                    messageString = await _telemetryMessagingService.GetMessageAsync();
